...
app.post('/SymAsymTester', (req, res) => {
    ...
    fs.writeFile(tempInputFile, ciphertext, (err) => {
        ...
        // Execute Test.py
        exec(`python backend/SymAsymTester/Test.py ${tempInputFile}`, (error, stdout, stderr) => {
            if (error) {
                console.error(`Error: ${error.message}`);
                res.status(500).json({ error: 'Error executing Python script.' });
                return;
...
app.post('/StreamBlockTester', (req, res) => {
    ...
    fs.writeFile(tempInputFile, ciphertext, (err) => {
        ...
        exec(`python backend/StreamBlockTester/Test.py ${tempInputFile}`, (error, stdout, stderr) => {
            if (error) {
                console.error(`Error: ${error.message}`);
                res.status(500).json({ error: 'Error executing Python script.' });
                return;
...
app.post('/StreamIdentifier', (req, res) => {
    ...
    fs.writeFile(tempInputFile, ciphertext, (err) => {
        ...
        exec(`python backend/StreamIdentifier/Test.py ${tempInputFile}`, (error, stdout, stderr) => {
            if (error) {
                console.error(`Error: ${error.message}`);
                res.status(500).json({ error: 'Error executing Python script.' });
                return;
...
app.post('/BlockIdentifier', (req, res) => {
    ...
    fs.writeFile(tempInputFile, ciphertext, (err) => {
        ...
        // Step 2: Execute the Python script to process the ciphertext
        exec(`python backend/BlockIdentifier/Test.py ${tempInputFile}`, (error, stdout, stderr) => {
            if (error) {
                console.error(`Error: ${error.message}`);
                res.status(500).json({ error: 'Error executing Python script.' });
                return;
